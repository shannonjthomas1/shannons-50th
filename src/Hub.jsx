...
const COLORS = {
  ...
  roseGold: "#B76E79", champagneGold: "#E5C158", cream: "#FAF3EE",
};
...
function QRCode({ url, label }) {
  return (
    <div style={{ background: COLORS.cream, borderRadius: 10, padding: "10px 12px", minWidth: 110, display: "flex", flexDirection: "column", alignItems: "center" }}>
      <img src={`https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(url)}&color=4B1D52&bgcolor=FAF3EE`}
        alt={`QR code for ${label}`} width={90} height={90} style={{ display: "block", borderRadius: 4 }} />
      <p style={{ fontSize: 9, color: COLORS.plumWine, marginTop: 5, textAlign: "center", fontWeight: 500 }}>Scan to {label}</p>
    </div>
  );
}
...
function HostGate({ onUnlock }) {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"48px 24px",textAlign:"center" }}>
      ...
      <p style={{ color:COLORS.mauveBlush,fontSize:13,marginBottom:20,maxWidth:280 }}>Enter the host passcode to manage the event.</p>
      <input type="password" placeholder="Passcode" value={input}
      ...
      <button onClick={attempt} style={{ background:COLORS.champagneGold,color:COLORS.deepPurple,border:"none",borderRadius:999,padding:"10px 28px",fontWeight:500,fontSize:14,cursor:"pointer" }}>Unlock</button>
      {err && <p style={{ color:"#E57373",fontSize:12,marginTop:8 }}>Incorrect passcode</p>}
    </div>
  );
}
...
function HostDashboard() {
  ...
  return (
    <div>
      <div style={{ display:"flex",gap:8,padding:"12px 16px",background:"rgba(0,0,0,0.2)",overflowX:"auto" }}>
        {views.map(v => (
          <button key={v.id} onClick={() => setHostView(v.id)} style={{
            flex:1,minWidth:90,padding:"8px 0",borderRadius:8,border:"none",cursor:"pointer",fontSize:12,fontWeight:500,whiteSpace:"nowrap",
...
export default function Hub() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase.from("guestbook_entries").select("name,text").eq("privacy","public").order("created_at",{ascending:false}).limit(20);
      if (data && data.length) setLiveEntries(data);
    };
    fetch();
    const ch = supabase.channel("hub_ticker")
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"guestbook_entries",filter:"privacy=eq.public"},
        p => setLiveEntries(prev => [p.new,...prev].slice(0,20)))
      .subscribe();
  ...
  return (
    <div style={{ minHeight:"100vh",width:"100%",display:"flex",flexDirection:"column",background:`linear-gradient(165deg,${COLORS.deepPurple} 0%,${COLORS.plumWine} 55%,${COLORS.deepPurple} 100%)`,fontFamily:"'Poppins',sans-serif" }}>
      ...
      <style>{`
        ...
        .tab-btn{flex:1;min-width:52px;display:flex;flex-direction:column;align-items:center;gap:2px;padding:8px 4px;border:none;background:transparent;cursor:pointer;transition:all 0.2s;border-bottom:2px solid transparent}
      ...
      <div style={{ background:"rgba(20,5,28,0.75)",borderBottom:"1px solid rgba(229,193,88,0.2)",backdropFilter:"blur(6px)",position:"sticky",top:0,zIndex:30 }}>
        <div className="tab-bar" style={{ maxWidth:700,margin:"0 auto" }}>
          {TABS.map(tab => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            return (
              <button key={tab.id} className={`tab-btn ${isActive?"active":""}`} onClick={() => setActiveTab(tab.id)}>
                <Icon size={17} style={{ color:isActive?COLORS.champagneGold:COLORS.mauveBlush }} strokeWidth={1.5}/>
                <span style={{ fontSize:9,fontWeight:500,color:isActive?COLORS.champagneGold:COLORS.mauveBlush,whiteSpace:"nowrap" }}>{tab.label}</span>
              </button>
            );
          })}
        </div>
      ...
      {currentTab.qr && (
        <div className="qr-row">
          <QRCode url={QR_URLS[activeTab]} label={currentTab.qrLabel} />
          <div>
            <p style={{ color:COLORS.champagneGold,fontSize:13,fontWeight:500,margin:"0 0 3px" }}>Scan to {currentTab.qrLabel}</p>
            <p style={{ color:COLORS.mauveBlush,fontSize:11,margin:0 }}>Or hand your phone to a friend</p>
          </div>
        </div>
      )}
