...
export default function BingoHost() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase.from("bingo_calls").select("number").order("created_at", { ascending: true });
      if (data) { const nums = data.map(d => d.number); setCalledNumbers(nums); if (nums.length) setLastCalled(nums[nums.length-1]); if (nums.length >= ALL_NUMBERS.length) setGameOver(true); }
    };
    fetch();
  }, []);
  ...
  const resetGame = async () => {
    ...
    setCalledNumbers([]); setLastCalled(null); setGameOver(false);
  };
